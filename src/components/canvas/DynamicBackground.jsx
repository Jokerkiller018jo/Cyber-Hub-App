...
export default function DynamicBackground() {
    ...
    useEffect(() => {
        ...
        // Speed multiplier (speedVal range 10-100; lower is faster or standard scale)
        ...
        const leaves = Array.from({ length: leafCount }, () => ({
            ...
            speedY: (Math.random() * 1.2 + 0.6) * speedFactor,
            speedX: (Math.random() * 0.8 - 0.4) * speedFactor,
        ...
        const flakes = Array.from({ length: snowflakeCount }, () => ({
            ...
            y: Math.random() * height,
            r: Math.random() * 3 + 1,
            speedY: (Math.random() * 1.5 + 0.8) * speedFactor,
            swing: Math.random() * 2,
        ...
        const bokehs = Array.from({ length: bokehCount }, () => ({
            ...
            y: Math.random() * height,
            r: Math.random() * 35 + 20,
            vx: (Math.random() - 0.5) * 0.6 * speedFactor,
            vy: (Math.random() - 0.5) * 0.6 * speedFactor,
        ...
        const lineTracers = Array.from({ length: 18 }, () => ({
            ...
            speed: (Math.random() * 3 + 2) * speedFactor
        }));
        ...
        const clouds = Array.from({ length: 6 }, (_, i) => ({
            ...
            speed: (Math.random() * 0.4 + 0.2) * speedFactor
        }));

        /* ── Main Render Loop ─────────────────────────────────── */
        const render = () => {
            time += 0.015 * speedFactor;
            ...
            switch (bgType) {
                ...
                case 'moving-gradient': {
                    const gx1 = width * (0.5 + 0.4 * Math.sin(time * 0.5));
                    const gy1 = height * (0.5 + 0.4 * Math.cos(time * 0.7));
                    const gx2 = width * (0.5 + 0.4 * Math.cos(time * 0.6));
                    const gy2 = height * (0.5 + 0.4 * Math.sin(time * 0.4));
                ...
                case 'liquid-gradient': {
                    ...
                    for (let i = 0; i < 4; i++) {
                        const cx = width * (0.5 + 0.35 * Math.sin(time * 0.8 + i * 1.5));
                        const cy = height * (0.5 + 0.35 * Math.cos(time * 0.6 + i * 2));
                        const rad = (width + height) * 0.25;

                        const radGrad = ctx.createRadialGradient(cx, cy, 10, cx, cy, rad);
                        radGrad.addColorStop(0, `${accent}55`);
                        radGrad.addColorStop(0.5, i % 2 === 0 ? 'rgba(168, 85, 247, 0.25)' : 'rgba(6, 182, 212, 0.25)');
                ...
                case 'floating-orbs': {
                    ...
                    orbs.forEach(orb => {
                        ...
                        grad.addColorStop(0, `${accent}77`);
                        grad.addColorStop(0.6, `${accent}22`);
                ...
                case 'sliding-diagonals': {
                    ...
                    const offset = (time * 60) % (stripeWidth * 2);
                ...
                case 'animated-wave': {
                    ...
                    for (let layer = 0; layer < 4; layer++) {
                        ...
                        const speed = time * (1 + layer * 0.4);

                        for (let x = 0; x <= width; x += 15) {
                            const y = height * (0.65 - layer * 0.1) + Math.sin(x * freq + speed) * amp + Math.cos(x * freq * 0.5 + speed) * 20;
                            ctx.lineTo(x, y);
                        }
                ...
                case 'soft-clouds': {
                    ...
                    clouds.forEach(cloud => {
                        ...
                        grad.addColorStop(0.4, `${accent}15`);
                        grad.addColorStop(1, 'transparent');
                ...
                case 'falling-leaves': {
                    ...
                    leaves.forEach(leaf => {
                        leaf.y += leaf.speedY;
                        leaf.osc += 0.03;
                        leaf.x += leaf.speedX + Math.sin(leaf.osc) * 0.8;
                ...
                case 'gentle-ripples': {
                    ...
                    rippleCenters.forEach((center, idx) => {
                        for (let ring = 0; ring < 6; ring++) {
                            const radius = ((time * 40 + ring * 50 + idx * 80) % 350);
                    ...
                    for (let band = 0; band < 3; band++) {
                        ...
                        for (let x = 0; x <= width; x += 20) {
                            const y = height * (0.35 + band * 0.12) +
                                Math.sin(x * 0.004 + time * 0.8 + band) * 70 +
                ...
                case 'hyperspeed-space': {
                    ...
                    stars.forEach(s => {
                        ...
                        if (px >= 0 && px <= width && py >= 0 && py <= height) {
                            ...
                            ctx.lineWidth = Math.min(3, (1 - s.z / width) * 3);
                            ctx.stroke();
                        }
                    });
                    break;
                    ...
                    for (let m = 0; m < 5; m++) {
                        ...
                        for (let x = 0; x <= width; x += 20) {
                            const y = height * (0.5 + m * 0.08) +
                                Math.sin(x * 0.005 + time * 1.2 + m) * 60 +
